...
export default defineConfig(({ command, mode }) => {
  // Check if this is a GitHub Pages build based on the base path
  const isGitHubPagesBuild = process.argv.includes('--base=/ancient-history-trivia-pwa/');
  
  const plugins = [react()];
  
  // Only add PWA plugin for non-GitHub Pages builds to avoid service worker issues
